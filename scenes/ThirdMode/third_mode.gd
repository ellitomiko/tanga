extends Node2D

@onready var line_edit = $MovingObjects/TextEdit
@onready var no_sprite = $MovingObjects/No
@onready var moving_objects = $MovingObjects

@export var yes_texture: Texture2D = preload("res://test_textures/mode2/YES.png")

@export var max_speed: float = 150.0
@export var acceleration: float = 180.0

var current_speed: float = 0.0
var is_moving: bool = false

func _ready() -> void:
	line_edit.editable = false

func _process(delta: float) -> void:
	if is_moving:
		current_speed = move_toward(current_speed, max_speed, acceleration * delta)
		moving_objects.position.x -= current_speed * delta

func _on_button_start_game_pressed() -> void:
	is_moving = true
	
	line_edit.editable = true
	line_edit.grab_focus()



func _on_text_edit_text_submitted(new_text: String) -> void:
	print("Ввели:", new_text)  # для проверки
	
	if new_text == "45":
		no_sprite.texture = yes_texture
